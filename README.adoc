...
(You need only a single CD or DVD; because whatever else you want can be obtained via APT [Advanced Packaging Tool], once the base system is installed)
...
often jigdeo will find some unchanged packages from older Debian Releases that it can reuse.
+
* Mount old iso-image:
+
[source,bash]
...
----
+
If apt-get does not work, and your're behind a windows proxy or firewall, then see <<_fix_up_with_cntlm, the section of cNTLM>>.
